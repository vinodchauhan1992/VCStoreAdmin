...
import { UpdateStateRequestModel } from 'src/models/StatesModel'
...
import { getCountryFromCountryId } from 'src/utils/CommonUtils'
...
const EditState = (props: EditStateProps) => {
  ...
  // @ts-ignore
  const updatedStateResponse = useAppSelector(StatesReducer.selectUpdatedStateResponse)
  // @ts-ignore
  ...
  const defaultValues: UpdateStateRequestModel = {
    id: '',
    title: '',
    countryID: '',
    isDeleteable: false,
  ...
  const [values, setValues] = React.useState<UpdateStateRequestModel>(defaultValues)
  ...
  const handleTextFieldChange =
    (prop: keyof UpdateStateRequestModel) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const onUpdateClick = () => {
    ...
    dispatch({ type: 'UPDATE_STATE', payload: { jsonData: newValues, stateId: newValues.id } })
  }
  ...
  React.useEffect(() => {
    if (updatedStateResponse?.isCompleted) {
      if (updatedStateResponse?.succeeded) {
        resetForm()
        resetSelectedState?.()
        dispatch(StatesReducer.resetUpdatedStateResponse())
        callAllStatesApi()
      } else {
        dispatch(StatesReducer.resetUpdatedStateResponse())
      }
                ...
                <StyledTableRow>
                  <StyledTableCell>
                    <Typography style={{ color: 'white' }} variant='subtitle1' gutterBottom component='div'>
                      {`Update ${row?.title} state(ID: ${row?.id})`}
                    </Typography>
                  </StyledTableCell>
                </StyledTableRow>
              </TableHead>
